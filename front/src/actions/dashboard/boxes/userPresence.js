      ...
      try {
        ...
        usersWithPresence.forEach((user) => {
          if (user.last_house_changed) {
            user.last_house_changed_relative_to_now = dayjs(user.last_house_changed).locale(state.user.language).fromNow();
          }
        });
        store.setState({ usersWithPresence, DashboardUserPresenceGetUsersStatus: RequestStatus.Success });
      } catch (e) {
        store.setState({ DashboardUserPresenceGetUsersStatus: RequestStatus.Error });
      }
    }
  };
  return actions;
