...
const actions = store => ({
  async getBridges (state) {
    ...
    try {
      ...
      store.setState({
        bridges, philipsHueGetBridgesStatus: PhilipsHueGetBridgesStatus.Success 
      });
    } catch (e) {
      store.setState({
        philipsHueGetBridgesStatus: PhilipsHueGetBridgesStatus.Error, philipsHueGetBridgeError: e.message 
      });
    }
  }
});
