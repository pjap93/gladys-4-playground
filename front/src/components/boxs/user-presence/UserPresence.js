...
const UserPresence = ({ children, ...props }) => (
  <div class="card">
    <div class="card-header"><h3 class="card-title"><i class="fe fe-home" /><span class="m-1"><Text id="dashboard.boxTitle.user-presence" /></span></h3>
    </div>
    { props.DashboardUserPresenceGetUsersStatus === RequestStatus.Error && <div class="card-alert alert alert-danger mb-0">
      <Text id="dashboard.boxes.userPresence.error" />
    </div>
    }
    <div class="card-body o-auto" style={{ maxHeight: '15rem', padding: '1rem' }}>
      <div class={props.DashboardUserPresenceGetUsersStatus === RequestStatus.Getting && !props.usersWithPresence ? 'dimmer active' : 'dimmer'}>
        <div class="loader" />
...
class UserPresenceComponent extends Component {
  ...
  componentDidMount() {
    ...
    this.props.session.dispatcher.addListener(WEBSOCKET_MESSAGE_TYPES.USER_PRESENCE.BACK_HOME, (payload) => this.props.userChanged(payload));
    this.props.session.dispatcher.addListener(WEBSOCKET_MESSAGE_TYPES.USER_PRESENCE.LEFT_HOME, (payload) => this.props.userChanged(payload));
  }
